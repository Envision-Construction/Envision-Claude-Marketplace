...
- `${CLAUDE_PLUGIN_ROOT}/skills/specialist/references/cell-structures.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/specialist/references/current-legislation.md` — check its as-of date; if older than 90 days or silent on a domicile fact you need (capital minimums, premium tax, cell provisions), verify against the domicile's official statute/regulator yourself.
- `${CLAUDE_PLUGIN_ROOT}/skills/specialist/references/forensic-research-protocol.md` — its rules bind you.
...
- Federal overlay: cell-by-cell insurance-status testing (Rev. Rul. 2008-8 line) and per-cell 831(b) mechanics belong in any cell recommendation; coordinate with captive-tax findings when both are dispatched.
